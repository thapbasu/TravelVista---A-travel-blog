...
module.exports.user_comment = async (req, res) => {
  const { articleId, articleSlug, commentText, userName, userImage } = req.body;
  ...
  try {
    ...
    await notificationModel.create({
      subject: `${userName} commented on your article`,
      slug: articleSlug,
      adminId,
    });
    res.status(201).json({
  ...
  if (role === 'admin') {
    ...
    try {
      ...
      if (getArticle.adminId === adminId) {
        ...
        res
          .status(200)
          .json({ successMessage: 'Comment reply delete success' });
      } else {
        res.status(401).json({
          errorMessage: {
