...
module.exports.mostLikedPosts = async (req, res) => {
  try {
    const mostLikedArticles = await articleModel.aggregate([
      {
        $sort: {
          likes: -1, // Sort in descending order of likes
        },
      },
      {
        $limit: 5, // Limit the result to the top 3 most liked articles
      },
    ]);

    return res.status(200).json({ mostLikedArticles });
  } catch (error) {
    return res.status(500).json({
      errorMessage: {
        error: 'Internal server error',
