    ...
    try {
      ...
      if (getAdmin) {
        ...
        if (matchPassword) {
          ...
          return res
            .status(400)
            .json({ errorMessage: { error: 'Passwod does not match' } });
        }
      } else {
        return res
          .status(400)
...
module.exports.verify_email = async (req, res) => {
  ...
  if (!otp) {
    ...
    const disPath =
      __dirname + `../../../frontend/public/userImages/${imageName}`;
        ...
        fs.copyFile(imageInfo.image.filepath, disPath, async (err) => {
          if (!err) {
            const createUser = await userModel.create({
              ...
              password,
              image: `http://localhost:3000/userImages/${imageName}`,
            });
